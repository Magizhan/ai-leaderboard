...
async function logUsage(body, env) {
  ...
  if (history.length > MAX_HISTORY) {
    const oneDayAgo = Date.now() - 24 * 36e5;
...
async function getLeaderboardData(env) {
  ...
  const board = await Promise.all(users.map(async (u) => {
    ...
    const planTypeCost = /* @__PURE__ */ __name((pt) => pt === "max5" ? 100 : 200, "planTypeCost");
    ...
    const planScale = /* @__PURE__ */ __name((pt) => pt === "max5" ? 0.25 : 1, "planScale");
    let budget = 0;
    if (usage && usage.plans && usage.plans.length > 0) {
    ...
    let rawSessionPct = usage ? usage.combinedSessionPct !== void 0 ? usage.combinedSessionPct : usage.sessionPct || 0 : 0;
    ...
    if (history.length >= 2) {
      ...
      if (recent.length >= 2) {
        ...
        if (u.numPlans > 1) {
          ...
          const weeksElapsed = Math.max(1, Math.min(4, Math.ceil((nowMs - oldestRecentTs) / (7 * 864e5))));
        ...
        const weeksElapsed = Math.max(1, Math.min(4, Math.ceil(entryAge / (7 * 864e5))));
        displayWeeklyPct = (recent[0].weeklyPct || 0) / weeksElapsed * scale;
      }
    }
    if (displayWeeklyPct === 0 && usage) {
      const rawWeekly = usage.combinedWeeklyPct !== void 0 ? usage.combinedWeeklyPct : usage.weeklyPct || usage.pct || 0;
      const lastSyncAge = usage.timestamp ? nowMs - new Date(usage.timestamp).getTime() : 28 * 864e5;
      const weeksElapsed = Math.max(1, Math.min(4, Math.ceil(lastSyncAge / (7 * 864e5))));
    ...
    return {
      ...
      lastUpdated,
      isStale: lastUpdated && Date.now() - new Date(lastUpdated).getTime() > 24 * 60 * 60 * 1e3,
      isInactive: lastUpdated && Date.now() - new Date(lastUpdated).getTime() > 72 * 60 * 60 * 1e3,
...
// .wrangler/tmp/bundle-bQntnL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bQntnL/middleware-loader.entry.ts
